...
function SignIn() {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      const res = await axios.post("https://localhost:7000/api/user/login", {
        email,
        password,
      });

      // Destructure userId, token, email from response
      const { userId, token, email: userEmail } = res.data;

      // Store all in sessionStorage
      sessionStorage.setItem("userId", userId);           // <-- userId stored here
